...
from config.rhd_config import snap_joint_names, SNAP_PARENT
...
class HandDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'joint' in sample.keys():
            ...
            kin_chain = [
                jointRS[i] - jointRS[SNAP_PARENT[i]]
                for i in range(21)
            ]
        ...
        if self.vis:
            ...
            plt.subplot(1, 4, 3)
            clr3 = my_clr2.copy().resize((self.hm_res, self.hm_res), Image.LANCZOS)
            tmp = clr3.convert('L')
            tmp = np.array(tmp)
            ...
            # plt.show()
            plt.savefig('rhd' + str(random.randint(1, 10)) + '.png')
        ...
        metas = {
            'index': index,
            'rgb': clr, # rgb image "clr"
            'hm': hm,
            'hm_veil': hm_veil,
            'location_map': location_map,
            'delta_map': delta_map,
            'flag_3d': flag,
            "rel_3d": joint, # 3d joint "joint"
            "rel_2d": kp2d, # 2d keypoint "kp2d"
            "scale": scale, # scale
        }
...
if __name__ == '__main__':
    train_set = HandDataset(
        ...
        max_rot=0.5 * np.pi,
        subset_name=["rhd"],
        data_root="/root/autodl-tmp", vis=True
    )

    for id in tqdm(range(0, len(train_set), 10)):
        print("id=", id)
        data = train_set[id]