...
# import rhd_config as cfg
from VAEPose.VaePose.config.rhd_config import DEFAULT_CACHE_DIR, snap_joint_names, rhd_joints
import VAEPose.VaePose.utils.handutils as handutils

CACHE_HOME = os.path.expanduser(DEFAULT_CACHE_DIR)

snap_joint_name2id = {w: i for i, w in enumerate(snap_joint_names)}
rhd_joint_name2id = {w: i for i, w in enumerate(rhd_joints)}
rhd_to_snap_id = [snap_joint_name2id[joint_name] for joint_name in rhd_joints]
...
def main():
    data_split = 'train'
    rhd = RHDDataset(
        data_root="/root/autodl-tmp/rhd/RHD_published_v2",
