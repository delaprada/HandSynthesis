...
def save_tex(fname, tex_img):
    import cv2
    fname = Path(fname)
    
    # texture image
    tex_name_diffuse = fname.parent / "diffuse.png"
    tex_img = np.uint8(tex_img * 255)
    
    cv2.imwrite(str(tex_name_diffuse), tex_img[:,:, :3])
    cv2.imwrite(str(fname.parent / "normal.png"), tex_img[:,:,3:6])
    cv2.imwrite(str(fname.parent / "spec.png"), tex_img[:,:,6:])
    
    print("save to", fname)

def save_textured_nimble(fname, skin_v, tex_img, save_obj):
    ### batch_size = 1
    import cv2
    ...
    obj_name_skin = fname.parent / "skin.obj"
...
def mano_v2j_reg(mano_verts):#[b,778,3]
        ...
        MANO_file = r'assets/MANO_RIGHT.pkl'
